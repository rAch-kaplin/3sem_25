...
static int set_sigaction(int signal, void (*handler)(int)) {
    struct sigaction sa = {};
...
void sig_exit_handler(int sig __attribute__((unused))) {
    ...
    set_sigaction(SIG_CONSUMER, SIG_IGN);
}
...
void producer_handler(int sig __attribute__((unused)), siginfo_t *info, void *ucontext __attribute__((unused))) {
    ...
    do {
        ...
        sh_data->attempts++;
    } while (curr_size == 0 && sh_data->bytes_read < sh_data->file_size &&
            sh_data->attempts < MAX_ATTEMPTS - 1);
    ...
    if (curr_size > 0) {
        ...
        set_sigaction(SIG_PRODUCER, SIG_IGN);
...
void consumer_handler(int sig __attribute__((unused)), siginfo_t *info, void *ucontext __attribute__((unused))) {
    ...
    while (bytes_written_total < bytes_to_write) {
        ssize_t n = write(sh_data->fd_out, buf_ptr + bytes_written_total, bytes_to_write - bytes_written_total);
